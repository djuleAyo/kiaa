...
void build_segment_tree (std::vector<int> &array, std::vector<Node> &segment_tree, int k, int x, int y)
{
  ...
  /********* C++ deo *********/
  // Funckija __gcd() iz standardne biblioteke racuna najveci zajednicki delilac za 2 broja koja se prosledjuju kao argumenti. Obratiti paznju na naziv funkcije (dve donje crte pa gcd)
...
Node get_number_of_dividers (std::vector<Node> &segment_tree, int k, int x, int y, int a, int b)
{
  ...
  if (left.greatest_common_divider && right.greatest_common_divider) {
    ...
    result.min_element = std::min(left.min_element, right.min_element);
    ...
    if (result.greatest_common_divider != result.min_element) {
      result.counter = 0;
    }
...
int main ()
{
  std::vector<int> array = { 5, 2, 3, 2, 6, 4, 7, 8 };
  ...
  result = get_number_of_dividers(segment_tree, 1, 0, n - 1, 3, 5);
